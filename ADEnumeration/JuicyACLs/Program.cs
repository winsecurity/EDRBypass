...
namespace juicyacls
{
    class Program
    {
        ...
        {
            // juicyacls -username username -domain domainname -server serverip
            ...
            if (username == null)
            {
                Console.WriteLine("Please supply -username DOMAIN\\username you own");
            ...
            //ds.SearchScope = SearchScope.Base | SearchScope.Subtree;
            //ds.Filter = "(samaccountname=test1)";
            ds.SearchRoot = de;

            foreach(SearchResult sr in ds.FindAll())
            {
                DirectoryEntry objentry =  sr.GetDirectoryEntry();
                try
                {
                    AuthorizationRuleCollection arc = objentry.ObjectSecurity.GetAccessRules(true, true, typeof(NTAccount));
                    ...
                    foreach (ActiveDirectoryAccessRule ace in arc)
                    {
...

                        {
                            ...
                            Console.WriteLine("Object: {0}", objentry.Name);
                            Console.WriteLine("Identity Reference: {0}", ace.IdentityReference.ToString());
                            Console.WriteLine("Access Control Type: {0}", ace.AccessControlType);
                            Console.WriteLine("Rights: {0}", ace.ActiveDirectoryRights);

                            Console.WriteLine("Right GUID: {0}", ace.ObjectType);
                            string extendedrightname = getextendedrightname(ace.ObjectType.ToString());
                            if (extendedrightname != null)
                            {
                                Console.WriteLine("Extended right name: {0}", extendedrightname);
