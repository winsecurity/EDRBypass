...
namespace get_reversableencryptionpasswordusers
{
    class Program
    {
        ...
        {
            // get_reversableencryptionpasswordusers   -domain domainname
            // -server serverip
            ...
            ds.Filter = "(useraccountcontrol>=127)";

            foreach (SearchResult sr in ds.FindAll())
            {
                int uac = Convert.ToInt32(sr.Properties["useraccountcontrol"][0]);

                List<string> uacstrings = getuacvalues(uac);
                foreach (string uacstring in uacstrings)
                {
                    if (uacstring.ToUpper() == "ENCRYPTED_TEXT_PWD_ALLOWED")
                    {
                        Console.WriteLine("[+] {0}",sr.Properties["distinguishedname"][0]);
                        Console.WriteLine();
                    }
                }
