...
myconnection.search(args.searchbase if args.searchbase else namingcontext,
                    "(&(objectclass=user)(useraccountcontrol>=16777215))",
                    "SUBTREE",attributes=["useraccountcontrol","name","msDS-AllowedToDelegateTo"])
...
for i in range(0,len(myconnection.entries)):
    ...
    for j in l:
        if "useraccountcontrol" in j.lower():
            ...
            for k in uacflags:
                if k=="TRUSTED_TO_AUTH_FOR_DELEGATION":
...
myconnection.search(args.searchbase if args.searchbase else namingcontext,
                    ...
                    "SUBTREE",attributes=["msDS-AllowedToDelegateTo","name"])
