...
namespace Find_ESC2
{
    class Program
    {
        ...
        public static List<string> checkiflowusercanenroll(AuthorizationRuleCollection arc)
        {
            ...
            highpriv.Add("domain controller");
        ...
        static void Main(string[] args)
        {
            ...
            ds.PropertiesToLoad.Add("msPKI-Certificate-Application-Policy");*/
            ...
            foreach(SearchResult sr in ds.FindAll())
            {
                ...
                int numberofsignaturesrequired = Convert.ToInt32(sr.Properties["msPKI-RA-Signature"][0]);
                if (numberofsignaturesrequired !=0)
                {
                    continue;
                }


                // CT_FLAG_PEND_ALL_REQUESTS must not be in enrollment flags
                // this indicates our requests wont be in pending state inorder for manager to approve
                bool managerapprovalrequired = false;
                List<string> enrollmentflags = getEnrollmentFlags(Convert.ToInt32(sr.Properties["msPKI-Enrollment-Flag"][0]));
                foreach(string flag in enrollmentflags)
                {
                    if (flag.Contains("CT_FLAG_PEND_ALL_REQUESTS"))
                    {
                        managerapprovalrequired = true;
                        break;
                    }
                }
                if (managerapprovalrequired == true)
                {
                    continue;
                }
                ...
                // search application policy and pki extended usage
                // for "Any purpose" eku

                List<string> oids = new List<string>();
                bool isanyekupresent = false;

                try
                {
                    if (sr.Properties["msPKI-Certificate-Application-Policy"].Count > 0)
                    {
                        foreach (string oid in sr.Properties["msPKI-Certificate-Application-Policy"])
                        {
                            var a = getKeyUsage(oid);
                            if (a != null)
                            {
                                oids.Add(a);
                            }
                ...
                try
                {
                    ...
                    {
                        foreach (string oid in sr.Properties["pKIExtendedKeyUsage"])
                        {
                            var a = getKeyUsage(oid);
                            if (a != null)
                            {
                ...
                if (oids.Count > 0)
                {
                    ...
                    {
                        if (oid.Contains("any purpose"))
                        {
                            isanyekupresent = true;
                            break;
                        }
                    }
                ...
                List<string> aces = checkiflowusercanenroll(ads.GetAccessRules(true,true,typeof(NTAccount)));


                if (aces.Count > 0)
                {
                    Console.WriteLine("Template Name: {0}",sr.Properties["name"][0]);
                    Console.WriteLine("Who can enroll: {0}",String.Join(", ",aces));
                    Console.WriteLine();
                }
